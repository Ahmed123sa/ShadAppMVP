...
export default function ContractBuilder({ wsId, onCreated, onCancel }: { wsId: number; onCreated: (contract: any) => void; onCancel: () => void }) {
  ...
  return (
    <div className="border rounded-xl bg-zinc-50 p-4 space-y-3">
      <h3 className="text-sm font-bold text-zinc-700">عقد خدمة إضافي</h3>
      ...
      {fixedTemplates.length > 0 && (
        <div className="border rounded p-3 bg-white">
          <h4 className="text-xs font-bold text-zinc-500 mb-2">بنود إلزامية</h4>
      ...
      <div className="flex gap-2">
        <button onClick={create} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">إنشاء وإرسال</button>
        <button onClick={onCancel} className="bg-zinc-200 text-zinc-600 px-4 py-2 rounded-lg text-sm hover:bg-zinc-300">إلغاء</button>
      </div>
    </div>
  );
}
